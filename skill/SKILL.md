...
# 把输出的 JS 数组粘贴到 scenes/scene-N.html 的 SUBTITLES 位置
```
...
**Q: 字幕时间不准确？**
A: 调整对应 `scenes/scene-N.html` 中 `SUBTITLES` 数组的 `start` / `end` 即可。算法 (与原 SRT 一致) 在 video-generation.md 第 3 节有说明。避免"每句固定2秒"的简单方案。
