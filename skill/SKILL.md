...
> 以下 1080×1920 为默认值；在 `script.json` 设 `"video": {"width": 1080, "height": 1440}` 可切换 3:4 等比例，viewport/body/scene 尺寸由工具自动注入。
...
| 小红书 | 1080×1440 | 3:4 | script.json 设 video 原生输出（或 FFmpeg 裁剪顶底240px） |
...
- [ ] 分辨率匹配 `script.json` 的 `video` 配置（默认 1080×1920），`overflow: hidden`
...
**Q: 如何适配小红书 3:4 比例？**
A: 推荐：在 `script.json` 设 `"video": {"width": 1080, "height": 1440}`，工具原生按 3:4 生成 viewport/截图/编码，无需裁剪。备选：HTML 仍按 1080×1920 设计，FFmpeg 裁剪 `-vf crop=1080:1440:0:240`（居中裁剪）。
