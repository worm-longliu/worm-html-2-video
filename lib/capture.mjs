...
import { mkdirSync, rmSync, existsSync, statSync, readdirSync, readFileSync } from 'fs';
...
const OUTPUT_FPS = 30;
// Video dimensions: read from script.json "video" config (default 1080x1920).
function readVideoDims() {
  const scriptPath = join(process.cwd(), 'script.json');
  let width = 1080;
  let height = 1920;
  try {
    if (existsSync(scriptPath)) {
      const script = JSON.parse(readFileSync(scriptPath, 'utf-8'));
      const video = (script && script.video) || {};
      if (Number.isInteger(video.width) && video.width > 0) width = video.width;
      if (Number.isInteger(video.height) && video.height > 0) height = video.height;
    }
  } catch {
    // script.json missing/invalid → keep 1080x1920 defaults (back-compat)
  }
  return { width, height };
}
const { width: WIDTH, height: HEIGHT } = readVideoDims();
