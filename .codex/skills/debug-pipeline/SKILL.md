...
4. **先复现，再修复** — 用 7 步流水线（见 `README.md` Quick Start）作为 reproducer。
...
7 步流水线可逐段独立执行，按 segment 定位：

| 阶段 | 工具 | 快速验证 |
|---|---|---|
| 1-2. 脚本 / HTML | 浏览器预览（空格播放、方向键逐帧、数字键跳场景） | `python -m http.server` 后手播 |
| 3. 配音 | `python lib/generate_video.py --voiceover-only` | `ffprobe voiceover.mp3` 看时长 |
| 4. 字幕 | `python lib/generate_video.py --subtitles-only` | 查 `subtitle.srt` 条目与时间 |
| 5. 调时 | 重跑 `node lib/capture.mjs` | `ffprobe video_html.mp4` 看时长 |
| 6. 合成 | `python lib/generate_video.py` | 查 stderr / `video_final.mp4` |
...
3. 为何 `voiceover.mp3` 不存在？— `--voiceover-only` 没跑或 Edge-TTS 失败
