...
## 音视频合成视角（lib/voiceover.py + lib/generate_video.py + Edge-TTS + FFmpeg）
...
7. **TTS 配置** — `--voice` / `--rate` 是否覆盖默认？默认值改了是否同步更新 README 与 `skill/SKILL.md`？
8. **配音复用** — `generate_video.py` 合成阶段是否优先复用已存在的 `voiceover.mp3`（`--no-voiceover`）？

## CLI 入口视角（bin/cli.js）

1. **命令面** — `init` / `script` / `voiceover` / `sync` / `capture` / `generate` 六个子命令的边界是否清晰？新增子命令是否同步更新 `--help` 文案与 `README.md` CLI 表格？
